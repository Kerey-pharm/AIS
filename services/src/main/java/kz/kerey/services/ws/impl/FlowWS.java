...
public class FlowWS implements FlowInterface {
	...
	public void deleteStep(
			@WebParam(name="ladderId")
			Long ladderId,
			@WebParam(name="id")
			Long id) throws WebServiceException {
		try {
			if (ladderId==null || ladderId==0)
				throw new WebServiceException(Constants.objectIsNull, "ladderId is null or empty");
			if (id==null || id==0)
				throw new WebServiceException(Constants.objectIsNull, "ID is null or empty");
	...
	public void swapLadderSteps(
			...
			Long left, 
			@WebParam(name="right")
			Long right) throws WebServiceException {
		if (ladderId==null || ladderId==0)
			throw new WebServiceException(Constants.objectIsNull, "ladderId ID is null or empty");
		if (left==null || left==0)
			throw new WebServiceException(Constants.objectIsNull, "left ID is null or empty");
		if (right==null || right==0)
			throw new WebServiceException(Constants.objectIsNull, "right ID is null or empty");
		try {
