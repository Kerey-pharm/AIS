	...
	public void updateElement(GoodWrapper objWas, GoodWrapper objNew) {
		if (objWas!=null && objNew!=null) {
			if (!objWas.getName().equals(objNew.getName())) {
				goodService.changeGoodProperty(objNew.getId(), GoodProperty.name, objNew.getName());
			}
			
			if (!objWas.getPartialSelling().equals(objNew.getPartialSelling())) {
				goodService.changeGoodProperty(objNew.getId(), GoodProperty.partialSelling, String.valueOf(objNew.getPartialSelling()));
			}
			
			if (!objWas.getCountPerBox().equals(objNew.getCountPerBox())) {
				goodService.changeGoodProperty(objNew.getId(), GoodProperty.countPerBox, String.valueOf(objNew.getCountPerBox()));
			}
			
			if (!objWas.getCountSellable().equals(objNew.getCountSellable())) {
				goodService.changeGoodProperty(objNew.getId(), GoodProperty.countSellable, String.valueOf(objNew.getCountSellable()));
			}
