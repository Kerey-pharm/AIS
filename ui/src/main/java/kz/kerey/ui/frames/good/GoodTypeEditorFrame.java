...
public class GoodTypeEditorFrame extends JFrame {
	...
	private GoodTypeEditPanel goodTypeEditPanel = new GoodTypeEditPanel(
			"Вернуть");
	private ListPanel goodTypeListPanel = new ListPanel("Производные",
			GoodTypeComboboxModel.getModel());
	...
	private void initComponents() {
		...
		goodTypeListPanel
				.setGoodListSelectionActionListener(new ListSelectionListener() {
					public void valueChanged(ListSelectionEvent e) {
						JList<GoodTypeWrapper> l = (JList<GoodTypeWrapper>) e
								.getSource();
		...
		goodTypeEditPanel.setSaveButtonActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				loader.updateElement(goodTypeEditPanel.getGoodWrapper(),
						goodTypeEditPanel.getUpdatedGoodWrapper());
				GoodTypeComboboxModel.getModel().reloadData();
			}
		});
