...
public class RoleEditorFrame extends JFrame {
	...
	private final ListPanel listPanel = new ListPanel("Роли", RoleComboModel.getModel());
	private final RoleEditPanel editPanel = new RoleEditPanel("Вернуть");
	private final JSplitPane splitter = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
	...
	private void initComponents() {
		...
		listPanel.setGoodListSelectionActionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				JList<RoleWrapper> l = (JList<RoleWrapper>) e.getSource();
		...
		editPanel.setSaveButtonActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				loader.updateElement(editPanel.getRoleWrapper(), editPanel.getUpdatedRoleWrapper());
